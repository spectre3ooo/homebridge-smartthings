    ...
    if (device.capabilities["Switch Level"] !== undefined) {
        if (device.commands.levelOpenClose) {
            ...
            if (device.capabilities["Color Control"] !== undefined) {
                ...
                thisCharacteristic.on('get', function(callback) { callback(null, Math.round((that.device.attributes.hue || 100)*3.6)); });
                thisCharacteristic.on('set', function(value, callback) { that.platform.api.runCommand(callback, that.deviceid, "setHue", { value1: Math.round(value/3.6) || 100 }); });
